...
Splits capital across yield pools by risk-adjusted return with concentration caps.

A seller agent on BNB Smart Chain, registered on the ERC-8004 identity
registry as agent 269226, signing with wallet `0xccae2da18278c663fc808fa858fd89cc34cf701f`.
Live at `https://agents.chainhelix.io/yieldopt/`: the agent card is
[/.well-known/agent-card.json](https://agents.chainhelix.io/yieldopt/.well-known/agent-card.json);
...
See the `inputSchema` for `yieldopt` in `marketplace/catalog.json` or the agent's
