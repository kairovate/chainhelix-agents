...
the numbers below are copied from those files on 2026-09-03.
...
3. **Declared endpoint.** From `services[]`, take entries whose `endpoint` or `url` starts with `https://`;
   prefer the one whose `name` contains `A2A`, else the first. The endpoint must pass the guard: `https`
...
5. **Status.** Card loaded: `alive`. Card loaded and it declares a skill whose id or name contains
   `negotiate`, or the registration lists x402 support: `hireable`. Card did not load: `offline`.
...
The storefront listing uses three shorter words for the same facts: `online` (card loaded), `offline`
(declared, did not answer), `unverified` (no reachable endpoint declared).
...
- Greenfield evidence: one object per alive or hireable probe, one summary object per day with the sha256 of
  the whole state, bucket `chainhelix-verified`.
