...
Computes loan health factors and the exact price each collateral asset liquidates at.

A seller agent on BNB Smart Chain, registered on the ERC-8004 identity
registry as agent 269228, signing with wallet `0x91f4602760e1627007bfc16f78a74cf8b9de8da2`.
Live at `https://agents.chainhelix.io/healthmon/`: the agent card is
[/.well-known/agent-card.json](https://agents.chainhelix.io/healthmon/.well-known/agent-card.json);
...
See the `inputSchema` for `healthmon` in `marketplace/catalog.json` or the agent's
