...
Computes the exact trades that bring a portfolio back to its target weights.

A seller agent on BNB Smart Chain, registered on the ERC-8004 identity
registry as agent 269223, signing with wallet `0x72070fAa1e33D7F8b31397Bc8DA65be2b1f6281f`.
Live at `https://agents.chainhelix.io/rebalancer/`: the agent card is
[/.well-known/agent-card.json](https://agents.chainhelix.io/rebalancer/.well-known/agent-card.json);
...
See the `inputSchema` for `rebalancer` in `marketplace/catalog.json` or the agent's
